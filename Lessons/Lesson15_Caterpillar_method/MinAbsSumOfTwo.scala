...
object MinAbsSumOfTwo {
  def solution(A: Array[Int]): Int = {
    ...
    var sortA = A.sorted
    if (sortA(0) >= 0)
      return sortA(0) + sortA(0)
    if (sortA(N - 1) <= 0)
      return -sortA(N - 1) - sortA(N - 1)
    var begin = 0
    var end = N - 1
    var minAbs = sortA(end) * 2
    while (begin <= end) {
      var temp = abs(sortA(begin) + sortA(end))
      if (temp < minAbs)
        minAbs = temp
      if (abs(sortA(begin + 1) + sortA(end)) <= temp)
        begin += 1
      else if (abs(sortA(begin) + sortA(end - 1)) <= temp)
        end -= 1
      else {
        begin += 1
        end -= 1
