...
object CountSemiprimes {
  def solution(N: Int, P: Array[Int], Q: Array[Int]): Array[Int] = {
    ...
    var semiprimenum = new Array[Int](N + 1)
    for (i <- 4 to N) {
      semiprimenum(i) = semiprimenum(i - 1)
      if (sieve(i) != 0 && sieve(i / sieve(i)) == 0)
        semiprimenum(i) += 1
    }
    val M = P.length
    var answer = new Array[Int](M)
    for (i <- 0 until M) {
      answer(i) = semiprimenum(Q(i)) - semiprimenum(P(i) - 1)
    }
    answer
  }
}